...
int activityStudies(const char *config){
  ...
  TH1D *h_MCT_dEdx             = new TH1D("h_MCT_dEdx","",100,0,7);  // dE/dx of the muon MCTracks [GeV]
  ...
  for(unsigned int iEvt = 0; iEvt < nEvts; ++iEvt){
    ...
    for(int iG4 = 0; iG4 < nGeant; ++iG4){
      ...
      // Now look at the corresponding MCTracks for the true dE/dx
      // First, find the current MCTrack w.r.t the G4 ID
      if(evt->no_mctracks == 0) continue;
      for(int iMCT = 0; iMCT < evt->no_mctracks; ++iMCT){
        int MCTId = evt->mctrk_TrackId[iMCT];

        // If it is, check if the reco track is the G4 track
        if(MCTId != id) continue;

        // Then, look at the dE/dx for each step
        for(int iStep = 0; iStep < evt->nmctrksteps[iMCT]; ++iStep){
          // Now fill the dEdx histogram
          // Make sure there is a deposition
          if(evt->mctrk_dEdx[iMCT][iStep] < std::numeric_limits<float>::epsilon()) continue;
          h_MCT_dEdx->Fill(evt->mctrk_dEdx[iMCT][iStep]*100);
        } // Step loop
      } // MCT loop
    }// Geant
  ...
  SetHistogramStyle1D(h_MCT_dEdx,"MCTrack dE/dx [MeV/cm]", "Rate");
  h_MCT_dEdx->Draw("hist");
  h_MCT_dEdx->SetLineWidth(3);
  h_MCT_dEdx->SetLineColor(kTeal-5);
  c1->SaveAs((location+"/MCT_dEdx"+tag+".png").c_str());
  c1->SaveAs((location+"/MCT_dEdx"+tag+".root").c_str());
  c1->Clear();
  ...
  h_qDep_cosDrift_BP->Scale(1,"width");
  h_qDep_cosDrift_BP->Draw("colz");
  c3->SaveAs((location+"/qDep_vs_cosDrift_BP"+tag+".png").c_str());
  c3->SaveAs((location+"/qDep_vs_cosDrift_BP"+tag+".root").c_str());
  c3->Clear();
